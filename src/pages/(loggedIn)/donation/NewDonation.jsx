...
const NewDonation = () => {
      ...
      switch (donorDetails.identityType) {
        case "PAN":
          ...
          if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(donorDetails.identityNumber)) {
            errors.identityNumber = "PAN number must be valid (e.g., ABCDE1234F)";
          }
          break;
        case "Driving License":

          if (!/^[A-Z]{2}[0-9]{2}[0-9]{13}$/.test(donorDetails.identityNumber)) {
            errors.identityNumber = "Driving license number must be valid (e.g., MH011234567890123)";
          }
          break;
        ...
        case "Aadhaar":
          ...
          if (!/^\d{12}$/.test(donorDetails.identityNumber)) {
            errors.identityNumber = "Aadhaar number must be a valid 12-digit number";
          }
          break;
        case "Passport":
          if (!/^[A-Z]{1}[0-9]{7}$/.test(donorDetails.identityNumber)) {
            errors.identityNumber = "Passport number must be valid (e.g., A1234567)";
          }
          break;
  ...
  const confirmPending = async () => {
    try {
      ...
      console.log(
        "Successfully created new pending donation:",
        donationResponse
      );
