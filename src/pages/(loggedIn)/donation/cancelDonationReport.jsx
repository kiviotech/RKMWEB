export const cancelDonationReport = (donations, reportType) => {
  // Filter to only include cancelled donations
  const cancelledDonations = donations.filter(
    (donation) => donation.attributes.status === "cancelled"
  );

  // Group donations based on mode first
  const groupedByMode = cancelledDonations.reduce((acc, donation) => {
  ...
  const grandTotal = cancelledDonations.reduce(
    (sum, donation) =>
