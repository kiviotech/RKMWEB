...
const ApplicationForm = () => {
  ...
  useEffect(() => {
    ...
    setFormData((prevState) => ({
      ...
      guests: prevState.guests.slice(0, formData.guestMembers).concat(
        Array.from(
          { length: formData.guestMembers - prevState.guests.length },
          () => ({
            guestName: "",
            guestAadhaar: "",
            guestNumber: "",
  ...
  return (
    <div className="application-form">
      ...
      {activeFormTab === 0 && (
        <ApplicationDetails goToNextStep={() => setActiveFormTab(1)} />
      )}
      {/* guest details form */}
      {activeFormTab === 1 && <GuestDetails />}

      {/* Visit details form */}

      {activeFormTab === 2 && <VisitDetails />}

      {activeFormTab === 3 && <VerifyDetails />}
    </div>
  );
};
